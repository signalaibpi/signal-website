...
- 2026-09-18: Swapped Implementation's color from teal-green to steel blue-gray (`#3e6478`) and Operation's from amber to rose (`#bd4b72`) — the original green/amber choices collided with the diagnostic's own RAG (Red/Amber/Green) status system (`Signal Business AI Diagnostic.md` → Output), which uses those same colors to mean risk status, not area. Strategy (blue) and Governance (violet) were unaffected and stayed as-is.
